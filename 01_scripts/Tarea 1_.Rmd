...
> Es importante cargar las bases de datos utilizando el proyecto en el que estamos trabajando como "escritorio", de otra manera tendremos problemas en cargar las bases en el caso de que queramos replicar el trabajo de otra persona.

```{r}
#Definimos la base de datos relacionada con los datos de las playlists de Going Seventeen
playlist_df <- read_csv("../02_data/youtube_playlist_GOSE_stats_complete.csv")


#Definimos la base de datos de los datos de mi historial personal relacionado con Going Seventeen
historial_df <- read_csv("../02_data/historial_personal_gose.csv")
```
...
> Si no vamos a utilizar nuevamente estas estadísticas, no es necesario guardar los datos en un objeto nuevo.
...
round((n_videos_que_he_visto/263) *100, 2)

```

> Una de las características de R Markdown es que no muestra los objetos si los guardas, por lo tanto, si quieres mostrar una estadística, **no es necesario guardar un objeto nuevo**.
...
ggplot(tf_playlist, 
  ...
  geom_text(aes(label = scales::comma(total_vistas)),
            hjust = -0.1,
            color = "black", size = 3) +
...
ggsave("../03_output/top10_videos+vistos_general_gose_plot.png",
       width = 10, height = 6)
```
...
ggsave("../03_output/top10_videos+vistos_personal_gose_plot.png", 
       width = 10, height = 6)
```
...
ggsave("../03_output/top10_fechas_gose_plot.png")
